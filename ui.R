ui <- fluidPage(
  tags$head(
    tags$meta(name="Content-Security-Policy", content="script-src 'self';"),
  ),
  fluidRow(includeHTML("header.html")), # include NIEHS header
  titlePanel("ToxCompl: Drugmatrix Completion"),
  sidebarLayout(
    sidebarPanel(style="overflow-y:scroll;", width=4,
      div(
        HTML("
        <h3>About</h3>
        <h4>Terminology</h4>
        ...
        <p>
          The data used in ToxCompl may be found <a href=\"https://doi.org/10.22427/NTP-DATA-500-110-002-000-9\">here</a>.
        </p>
    ...
    mainPanel(
      useShinyjs(),
      includeCSS("css/ntpweb-shared.css"),
  ...
  fluidRow(
    includeScript("js/minimal-no-frameworks/ntpweb-shared-minimal.js"),
    includeHTML("footer.html")
  ) # include NIEHS footer
)
